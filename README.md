...
All test scripts and test-only files are located in the `tests/` directory.
...
If you change `manage_images.py`, please run:

```console
$ bash tests/tests_for_manage_images.sh
```

These tests build and remove container images, so repeated runs can be slow. Before running them, it is strongly recommended to prepare the Docker and/or Podman build cache:

```console
$ bash tests/tests_heatup_caches.sh
```

You can also warm only one runtime:

```console
$ bash tests/tests_heatup_caches.sh --podman
$ bash tests/tests_heatup_caches.sh --docker
```

You can override the free-space check (not recommended!) with:

```console
$ bash tests/tests_heatup_caches.sh --force
```
...
```console
$ bash tests/tests_heatup_caches.sh --cleanup --podman
```

### Testing `build_linux.py`

If you change `build_linux.py`, please run:
